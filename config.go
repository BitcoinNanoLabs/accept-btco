...
type Config struct {
	...
	// Otherwise, accept-btco needs to poll the node for checking account balances.
	...
	MaxPayments int
	// Up to this amount underpayments are accepted. Amount in BTCO.
...
func (c *Config) Read() (err error) {
	*c = DefaultConfig
	...
	err = k.Load(env.Provider("ACCEPTBTCO_", ".", func(s string) string {
		return strings.Replace(strings.TrimPrefix(s, "ACCEPTBTCO_"), "_", ".", -1)
	}), nil)
	if err != nil {
		return
	}

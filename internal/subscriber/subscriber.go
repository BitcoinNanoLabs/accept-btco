...
type Subscriber struct {
	...
	newConnectionC      chan *btco.Websocket
...
func New(wsURL string, handshakeTimeout, writeTimeout, ackTimeout, keepaliveDuration time.Duration) *Subscriber {
	return &Subscriber{
		...
		newConnectionC:      make(chan *btco.Websocket),
...
func (s *Subscriber) sendWithTimeout(ws *btco.Websocket, msg btco.OutgoingMessage) error {
	msg.RequireAck()
	ch := make(chan struct{})
	s.m.Lock()

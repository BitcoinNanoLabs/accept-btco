...
func blockCreate(previous, account, representative, balance, link, key, work string) (block createdBlock, err error) {
	...
	var li []byte
	if strings.HasPrefix(link, "btco_") || strings.HasPrefix(link, "xrb_") {
	...
	// https://docs.btco.org/integration-guides/the-basics/#self-signed-blocks
...
func accountToPublicKey(address string) (public []byte, err error) {
	...
	keyB32btco := "1111" + address[0:52]
	inputChecksum := address[52:]

	public, err = addressEncoding.DecodeString(keyB32btco)
	if err != nil {
		return nil, err
	}
